...
\part{Chemical Equilibrium}
	...
	\section{Overview}
		...
		\itl{Le Châtelier}. Lexically, an equilibrium reaction is represented with a double-hooked arrow, $\leftrightharpoons$, instead of a normal
	...
	\section{Equilibrium Constants}
		...
		\subsection{Gaseous Equilibrium Constant \texorpdfstring{\Kp{}}{Kp}}
			...
			\graphdiagram[1.0]{
				\begin{tikzpicture}
					\begin{axis}[
						...
						width			= 160mm,
						xtick			= {0.0, 4, 5, 6, 10},
						xticklabels		= {0, 10\sps{-3}, 1, 10\sps{3}, $\infty$},
					]
		...
		\subsection{Equilibrium Quotients \texorpdfstring{\Qc}{Qc} and \texorpdfstring{\Qp}{Qp}}
			...
			\begin{bulletlist}
				& $Q < K$; the current position is too far backward, and the system proceeds in the \itl{forward} direction until equilibrium
				is achieved

				& $Q = K$; the system is currently at equilibrium, and will remain so unless disturbed

				& $Q > K$; the current position is too far forward, and the system proceeds in the \itl{reverse} direction until equilibrium
				is achieved.
			\end{bulletlist}
	...
	\section{Factors Affecting Equilibrium Position}
		...
		\subsection{Changes in Total (System) Pressure}
			...
			\txtreactioneqn{
				\ch{N2 \stG}\hspace{2mm} + \hspace{2mm}\ch{3 H2 \stG} \arrow{<=>} \ch{2 NH3 \stG}
			}
			...
			\txtreactioneqn{
				\ch{H2 \stG}\hspace{2mm} + \hspace{2mm}\ch{Br2 \stG} \arrow{<=>} \ch{2 HBr \stG}
			}
		...
		\subsection{Presence of Catalyst}
			...
			Thus, catalysts only change the amount of \itl{time taken} to reach equilibrium, not its position.
	...
	\section{The Process of Haber-Bosch}
		...
		\subsection{Process}
			...
			\subsubsection{Conditions}
				...
				\vspace{1.5em}
				\parbox{\textwidth}{
					\textbf{Conditions:}\tabto{35mm}    \SI{450}{\celsius}, \SI{250}{\atm}, Finely divided \ch{Fe} catalyst.
				}
