...
\part{Reaction Kinetics}
	...
	\section{Rates of Reaction}
		...
		R is the rate of the reaction (in \si{\mole\per\cubic\deci\metre\per\second}). k is the constant of proportionality
		...
		\subsection{Rate Constant}
			...
			Since the concentration of the reactants is represented in \si{\mole\per\cubic\deci\metre}, and the final rate of the reaction is
			represented with \si{\mole\per\cubic\deci\metre\per\second}, the rate constant must have appropriate units to ``balance'' the units
	...
	\section{Determination of Reaction Order}
		...
		\subsection{Initial Rates Method}
			...
			Given that the initial rates R\sbs{1} and R\sbs{2} are known, basic math can be applied to solve for \textit{x}, \textit{y},
			...
			\subsubsection{Zeroth Order Reactions}
				...
				\begin{tikzpicture}
					...
					\begin{axis}[
						axis lines		= left,
						domain			= 0:10,
						xlabel			= \textbf{[A]/\si{\mol\per\cubic\deci\metre}},
						ylabel			= \textbf{rate/\si{\mol\per\cubic\deci\metre\per\second}},
						axis line style	= very thick,
						height			= 80mm,
						width			= 120mm,
						xtick			= \empty,
